"""
env_shared_old.py — ESKİ SÜRÜM (obstacles_on_route öncesi)
Yedek: Engeller grid'e rastgele yerleşiyordu, rotaya değil.
"""
...
class DroneSwarmSharedEnv(gym.Env):
    ...
    def _get_obs(self):
        ...
        for i in range(self.N_DRONES):
            ...
            drone_obs = np.concatenate([
                pos_norm, vel_norm, target_norm, formation_err, rays,
            ]).astype(np.float32)
    ...
    def _apply_wall_sliding(self, pos: np.ndarray, vel: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    def _generate_obstacles(self, n_obs, start_center, target):
        """ESKİ: Engeller grid üzerinde rastgele, rotaya özel yerleştirme yok."""
        obstacles = []
